...
void DataStructures::formGlobalECs() {
    ...
    if (!ECGlobals.empty()) {
        ...
        for (std::vector<const GlobalValue*>::iterator GI = ECGlobals.begin(), E = ECGlobals.end(); GI != E; GI++) 
        {
            DEBUG(errs() << (*GI)->getName() << "\n");
        }
