...
void DSGraph::buildCallGraph(DSCallGraph& DCG, std::vector<const Function*>& GlobalFunctionList, bool filter) const {
    ...
    for (FunctionListTy::const_iterator ii = Calls.begin(),
        ...
        if (ii->isDirectCall()) {
            ...
            //
            // For each Possible target for this callsite

            errs() << "\tcallsite: " << *CS.getInstruction() << "\n";

            for (std::vector<const Function*>::iterator Fi = MaybeTargets.begin(), Fe = MaybeTargets.end(); Fi != Fe; ++Fi)
            {
                
                errs()  <<  "\t\t" << (*Fi)->getName();
                // check if this function is possible
            ...
            for (DSCallSite::MappedSites_t::iterator I = ii->ms_begin(), E = ii->ms_end(); I != E; ++I) 
            {
                CallSite MCS = *I;
                for (std::vector<const Function*>::iterator Fi = MaybeTargets.begin(), Fe = MaybeTargets.end(); Fi != Fe; ++Fi)
                {
