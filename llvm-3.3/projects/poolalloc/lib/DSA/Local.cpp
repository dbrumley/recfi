...
bool LocalDataStructures::runOnModule(Module &M) {
    ...
    for (Module::iterator I = M.begin(), E = M.end(); I != E; ++I)
        if (!I->isDeclaration()) {
            ...
            errs() << "Calculating function: " << I->getName() << "\n";
            // Create new graph from globals
            DSGraph* G = new DSGraph(GlobalECs, getDataLayout(), *TypeSS, GlobalsGraph);
            // ?? Build the local graph of a function
            GraphBuilder GGB(*I, *G, *this);
            // ?? Set AuxCalls = Calls
            G->getAuxFunctionCalls() = G->getFunctionCalls();
            //??  Associate the graph with this function
            setDSGraph(*I, G);
            // ?? Propogate Unkown Flags
            ...
            // Union globals into equivalence classes
            errs() << "\t..form site 2\n";
            formGlobalECs();
            // Assert OK
